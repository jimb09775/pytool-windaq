...
Y_SCALE_IN         = 1.0     # fixed Y axis range: +/- this value in inches
MM_PER_INCH        = 25.4
# ──────────────────────────────────────────────────────────────────────────────
...
def load_wdh(filepath: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    x_in      = (v2 - STRINGPOT_OFFSET_V) * 1000.0 / MM_PER_INCH
    sensor_in = (v1 - sensor_offset)      * 1000.0 / MM_PER_INCH
    y_in      = x_in - sensor_in
...
def multiplot(filepaths: list[str]) -> None:
    ...
    for i, fp in enumerate(filepaths):
        ...
        x_in, y_in = load_wdh(fp)
        valid = np.isfinite(y_in)

        ax.scatter(x_in, y_in, s=1, alpha=0.35, color=color,
                   rasterized=True, label=label)

        mean = y_in[valid].mean()
        std  = y_in[valid].std()
        mn   = y_in[valid].min()
        mx   = y_in[valid].max()
        n    = valid.sum()
        summary_lines.append(
            ...
            f'min={mn:+.3f}  max={mx:+.3f}  n={n}'
        )
        print(f'{label}: mean={mean:+.3f} in  std={std:.3f} in  '
              f'min={mn:+.3f}  max={mx:+.3f}  n={n}')
